...
object EncryptedScriptFileHeader {
    ...
    fun getHeaderFlags(file: File): Short {
        ...
        return if (fis.read(bytes) < BLOCK_SIZE) {
            FLAG_INVALID_FILE
        } else if (!isValidFile(bytes)) {
            FLAG_INVALID_FILE
        } else {
            (bytes[BLOCK.size].toInt() * 256 + bytes[BLOCK.size + 1]).toShort()
        }
    }
    ...
    fun isValidFile(bytes: ByteArray): Boolean {
        for (i in BLOCK.indices) {
            if (bytes[i] != BLOCK[i]) return false
        }
        return true
    }

    fun writeHeader(os: java.io.OutputStream, flags: Short = 0) {
        os.write(BLOCK)
        os.write(flags.toInt() / 256)
        os.write(flags.toInt() % 256)
    }
}

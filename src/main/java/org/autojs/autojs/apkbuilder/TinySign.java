...
public class TinySign {
    ...
    // ! Rhino 1.8.1-SNAPSHOT requires dynamically loading services during
    // initialization (e.g., org.mozilla.javascript.RegExpLoader).
    // ! When loading these services, it needs to read the service provider
    // configuration files located in the META-INF/services/ directory.
    // ! During signing, these configuration files need to be written into the
    // ZipOutputStream.
    // !
    // ! Refer to:
    // https://stackoverflow.com/questions/4544899/java-meta-inf-services
    // !
    // ! zh-CN:
    // !
    // ! Rhino 1.8.1-SNAPSHOT 在初始化时需要动态加载服务 (如 org.mozilla.javascript.RegExpLoader),
    // ! 这些服务加载时, 需要读取位于 META-INF/services/ 目录下的服务提供者配置文件 (Service Provider
    // Configuration Files).
    ...
    private static byte[] writeSF(ZipOutputStream zos, Manifest sf, String sha1Manifest) throws Exception {
        ...
        out.write(("Created-By: tiny-sign-"
                + Objects.requireNonNull(TinySign.class.getPackage()).getImplementationVersion() + "\r\n")
                .getBytes(StandardCharsets.UTF_8));

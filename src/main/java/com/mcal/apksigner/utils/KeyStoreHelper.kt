...
object KeyStoreHelper {
    ...
    fun loadBks(bksFile: File?, password: CharArray): KeyStore {
        val keyStore: KeyStore
        try {
            // 确保在使用前初始化 SpongyCastle Provider
            val bcProvider = provider

            // SpongyCastle 使用 "SC" 作为 Provider 名称（而非 BouncyCastle 的 "BC"）
            keyStore = if (Security.getProvider("SC") != null) {
                KeyStore.getInstance("BKS", "SC")
            } else if (Security.getProvider("BC") != null) {
                KeyStore.getInstance("BKS", "BC")
            } else {
                // 直接使用已注册的 provider 对象
                KeyStore.getInstance("BKS", bcProvider)
            }
